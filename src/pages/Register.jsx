...
const Register = () => {
  ...
  const [loading, setLoading] = useState(false); // ✅ Loading state takia baar-baar click na ho
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // ✅ FIX: Route ko '/api/user/register' se badal kar '/api/auth/register' kiya hai
      ...
      // Successfully sign up ke baad login page par redirect
      navigate("/");
    } catch (err) {
  ...
  // ✅ Future Google Auth handler ke liye function bana diya hai
  const handleGoogleSignup = () => {
    alert("Google integration step complete hote hi ye active ho jayega! 😉");
  };
      ...
      <div className="hidden lg:flex w-1/2 relative items-center justify-center px-16">
        <div className="relative z-10">
          ...
          <h2 className="text-6xl font-bold leading-tight mt-8">
            Create <span className="block text-green-400">Job Winning</span> Resumes
          </h2>
        ...
        <div className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8">
          ...
          <form onSubmit={handleRegister} className="mt-8 space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-green-600 hover:bg-green-700 disabled:bg-green-800 py-3 rounded-2xl font-semibold transition"
            >
